...
struct fuse_conn {
	...
	/* Use P2P DMA for GPU Direct Storage */
	unsigned int gds:1;
	...
	/** Called on conn initialization to set limits */
	void (*init)(struct fuse_conn *);

	/** Called on fuse_conn_abort if connected */
	void (*abort)(struct fuse_conn *);

	/** Called on fuse_resend if connected */
	void (*resend)(struct fuse_conn *);

	/** Called after fuse_conn_abort */
	void (*wait_aborted)(struct fuse_conn *);

	/** Called on before final put */
	void (*conn_put_priv)(void *);
...
struct fuse_mount {
	 ...
	 * Send simple request
	 */
	ssize_t (*simple_request)(struct fuse_mount *, struct fuse_args *);
	/**
	 * Send background request
	 */
	int (*simple_background)(struct fuse_mount *, struct fuse_args *, gfp_t);
	/**
	 * Send FORGET command
	 */
	void (*forget_send)(struct fuse_mount *, struct fuse_forget_link *);
	/**
	 * Send notify reply command
	 */
	int (*simple_notify_reply)(struct fuse_mount *, struct fuse_args *, u64);
};
