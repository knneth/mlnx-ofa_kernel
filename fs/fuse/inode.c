...
static void fuse_cleanup_submount_lookup(struct fuse_mount *fm,
					 struct fuse_submount_lookup *sl)
{
	if (!refcount_dec_and_test(&sl->count))
		return;

	fuse_queue_forget(fm, sl->forget, sl->nodeid, 1);
	sl->forget = NULL;
	kfree(sl);
}
...
static void fuse_evict_inode(struct inode *inode)
{
	...
	if (inode->i_sb->s_flags & SB_ACTIVE) {
		...
		if (fi->nlookup) {
			fuse_queue_forget(fm, fi->forget, fi->nodeid,
					  fi->nlookup);
			fi->forget = NULL;
		}

		if (fi->submount_lookup) {
			fuse_cleanup_submount_lookup(fm, fi->submount_lookup);
			fi->submount_lookup = NULL;
		}
		/*
			       ...
			       int error)
{
	...
	if (error || arg->major != FUSE_KERNEL_VERSION)
		...
		if (arg->minor < 39) {
			/* FUSE_STATX was introduced in 7.39 */
			fc->no_statx = 1;
		}
