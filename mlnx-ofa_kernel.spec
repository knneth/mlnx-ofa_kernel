...
%{!?_release: %global _release OFED.26.04.1.1.0.1}
...
Utilities rpm.
The driver sources are located at: http://www.mellanox.com/downloads/ofed/mlnx-ofa_kernel-26.04-1.1.0.tgz
...
%post dkms
# Clean up broken old DKMS registrations before installing new version.
for ver_dir in /var/lib/dkms/%{name}/*/; do
...
%postun dkms
if [ $1 -eq 0 ]; then
	for old_link in %{_prefix}/src/ofa_kernel/%{_arch}/*; do
		[ -L "$old_link" ] && update-alternatives --remove ofa_kernel_headers "$old_link" || :
	done
	# Clean up stale directories to prevent DKMS rebuild failures on reinstall
	rm -rf %{_prefix}/src/ofa_kernel
	rm -rf %{_prefix}/src/ofa_kernel-dkms
fi

%posttrans dkms
# Create backward compatibility symlinks from old location to new DKMS location
for new_dir in "%{_prefix}/src/ofa_kernel-dkms/%{_arch}"/*; do
	[ -d "$new_dir" ] || continue
	kver=$(basename "$new_dir")
	old_link="%{_prefix}/src/ofa_kernel/%{_arch}/$kver"
	mkdir -p "%{_prefix}/src/ofa_kernel/%{_arch}"
	case "$old_link" in "%{_prefix}/src/ofa_kernel"/*/*) rm -rf "$old_link";; esac
