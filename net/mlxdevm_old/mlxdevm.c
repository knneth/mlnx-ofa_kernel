...
int mlxdevm_register(struct mlxdevm *dev)
{
	...
	INIT_LIST_HEAD(&dev->rate_node_list);

	init_rwsem(&dev->port_list_rwsem);
	init_rwsem(&dev->rate_node_rwsem);
	mutex_init(&dev->lock);
...
void mlxdevm_unregister(struct mlxdevm *dev)
{
	...
	WARN_ON(!list_empty(&dev->rate_node_list));
				...
				struct netlink_ext_ack *extack)
{
	...
	/* Reading a node here cannot result in use-after-free access of the node
	 * because this is done while holding the rate_node_rwsem. This serializes
	 * any ongoing node destruction from the driver layer.
	 */
	err = ops->rate_leaf_get(port, &tx_max, &tx_share, &node, extack);
	if (err)
		return err;
	...
	if (node) {
		if (nla_put_string(msg, MLXDEVM_ATTR_EXT_RATE_PARENT_NODE_NAME, node))
			goto nla_put_failure;
	}
				...
				struct netlink_ext_ack *extack)
{
	...
	if (nla_put_string(msg, MLXDEVM_ATTR_EXT_RATE_NODE_NAME, node->name))
		goto nla_put_failure;

	if (mlxdevm_nl_rate_fill_common(msg, MLXDEVM_RATE_EXT_TYPE_NODE,
					node->tx_max, node->tx_share))
		goto nla_put_failure;
...
static int mlxdevm_port_fn_leaf_node(struct mlxdevm_port *port,
				      const char *node, struct netlink_ext_ack *extack)
{
	const struct mlxdevm_ops *ops = port->devm->ops;

	if (!ops || !ops->rate_leaf_node_set) {
		NL_SET_ERR_MSG_MOD(extack,
				   "Function does not support leaf parent node setting");
		return -EOPNOTSUPP;
	}
	return ops->rate_leaf_node_set(port, node, extack);
}

static int mlxdevm_port_fn_node_tx_share(struct mlxdevm *dev, const char *node,
					 u64 tx_share, struct netlink_ext_ack *extack)
{
	const struct mlxdevm_ops *ops = dev->ops;
	...
	return ops->rate_node_tx_share_set(dev, node, tx_share, extack);
}

static int mlxdevm_port_fn_node_tx_max(struct mlxdevm *dev, const char *node,
				       u64 tx_max, struct netlink_ext_ack *extack)
{
	const struct mlxdevm_ops *ops = dev->ops;
	...
	return ops->rate_node_tx_max_set(dev, node, tx_max, extack);
}

static int mlxdevm_rate_node_get_doit_locked(struct mlxdevm *dev, struct genl_info *info,
					     struct sk_buff *msg)
{
	struct mlxdevm_rate_node *cur, *found_node;
	const char *node;
	int err;
	...
	node = nla_strdup(info->attrs[MLXDEVM_ATTR_EXT_RATE_NODE_NAME], GFP_KERNEL);
	if (!node)
		return -ENOMEM;

	down_read(&dev->rate_node_rwsem);
	found_node = NULL;
	list_for_each_entry(cur, &dev->rate_node_list, list) {
					  ...
					  struct netlink_callback *cb)
{
	...
	xa_for_each_marked(&mlxdevms, index, dev, MLXDEVM_REGISTERED) {
		down_read(&dev->rate_node_rwsem);
		list_for_each_entry(node, &dev->rate_node_list, list) {
			if (idx < start) {
				idx++;
				continue;
			}
			err = mlxdevm_nl_node_fill(msg, dev, node,
...
static int mlxdevm_cmd_rate_set_leaf(struct genl_info *info, struct mlxdevm *dev)
{
	...
	if (info->attrs[MLXDEVM_ATTR_EXT_RATE_PARENT_NODE_NAME]) {
		...
		node = nla_strdup(info->attrs[MLXDEVM_ATTR_EXT_RATE_PARENT_NODE_NAME], GFP_KERNEL);
		if (!node) {
			err = -ENOMEM;
			goto out;
		}

		err = mlxdevm_port_fn_leaf_node(port, node, extack);
		kfree(node);
	}
...
static int mlxdevm_cmd_rate_set_node(struct genl_info *info, struct mlxdevm *dev)
{
	...
	if (info->attrs[MLXDEVM_ATTR_EXT_RATE_TX_SHARE]) {
		rate = nla_get_u64(info->attrs[MLXDEVM_ATTR_EXT_RATE_TX_SHARE]);
		err = mlxdevm_port_fn_node_tx_share(dev, node, rate, extack);
		if (err)
			goto out;
	}
	...
	if (info->attrs[MLXDEVM_ATTR_EXT_RATE_TX_MAX]) {
		rate = nla_get_u64(info->attrs[MLXDEVM_ATTR_EXT_RATE_TX_MAX]);
		err = mlxdevm_port_fn_node_tx_max(dev, node, rate, extack);
	}
...
static const char *mlxdevm_node_get_node_name(struct genl_info *info)
{
	enum mlxdevm_rate_type rate_type;
	const char *node;
...
static int mlxdevm_nl_cmd_rate_new_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	node = mlxdevm_node_get_node_name(info);
	if (IS_ERR(node))
		return PTR_ERR(node);
	...
	err = ops->rate_node_new(dev, node, extack);
out:
	mutex_unlock(&mlxdevm_mutex);
	kfree(node);
	return err;
...
static int mlxdevm_nl_cmd_rate_del_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	err = ops->rate_node_del(dev, node, extack);
