				   ...
				   struct netlink_ext_ack *extack)
{
	...
	parent_ix = parent ? parent->ix : vport_node->esw->qos.root_tsar_ix;
...
static void __esw_qos_destroy_node(struct mlx5_esw_sched_node *node, struct netlink_ext_ack *extack)
{
	...
	if (node->type == SCHED_NODE_TYPE_VPORTS_TSAR ||
	    (node->type == SCHED_NODE_TYPE_TC_ARBITER_TSAR && !node->vport)) {
					 ...
					 struct mlx5_vport *vport)
{
	...
	if (!node_id) {
		...
		if (!vport->qos.sched_node->min_rate &&
...
static bool mlx5_devm_esw_vport_qos_check_and_disable(struct mlx5_vport *vport,
				      ...
				      struct netlink_ext_ack *extack)
{
	...
	if (mlx5_devm_esw_vport_qos_check_and_disable(vport, rate_leaf->parent,
						      rate_leaf->tx_max,
