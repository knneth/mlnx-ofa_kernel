...
{
	struct kmem_cache *htbls_cache = pool->dmn->htbls_kmem_cache;
	struct mlx5dr_icm_chunk *chunk;
	struct mlx5dr_ste_htbl *htbl;
	u32 num_entries;
	int i;

	htbl = kmem_cache_alloc(htbls_cache, GFP_KERNEL);
	if (!htbl)
		return NULL;
...
out_free_htbl:
	kmem_cache_free(htbls_cache, htbl);
	return NULL;
}

int mlx5dr_ste_htbl_free(struct mlx5dr_ste_htbl *htbl)
{
	struct kmem_cache *htbls_cache =
		htbl->chunk->buddy_mem->pool->dmn->htbls_kmem_cache;
