...
mlx5e_tc_post_act_init(struct mlx5e_priv *priv, struct mlx5_fs_chains *chains,
		       enum mlx5_flow_namespace_type ns_type)
{
	enum fs_flow_table_type table_type = ns_type == MLX5_FLOW_NAMESPACE_FDB_KERNEL ?
...
mlx5e_tc_post_act_add(struct mlx5e_post_act *post_act, struct mlx5_flow_attr *attr)
{
	...
	if (handle->ns_type == MLX5_FLOW_NAMESPACE_FDB_KERNEL)
...
mlx5e_tc_post_act_del(struct mlx5e_post_act *post_act, struct mlx5e_post_act_handle *handle)
{
	...
	if (handle->attr->action & MLX5_FLOW_CONTEXT_ACTION_EXECUTE_ASO) {
		mlx5_modify_header_dealloc(post_act->priv->mdev, handle->attr->modify_hdr);
		mlx5e_put_flow_meter(post_act->priv->mdev,
				     handle->attr->meter_attr.meters[0].handle);
	}
