		      ...
		      struct mlx5e_post_act_handle *handle)
{
	...
	err = mlx5e_tc_match_to_reg_set(mdev, &mod_acts, MLX5_FLOW_NAMESPACE_FDB_KERNEL,
					CHAIN_TO_REG, obj_id);
	if (err)
		goto err_set_regc0;
	...
	modify_hdr = mlx5_modify_header_alloc(mdev, MLX5_FLOW_NAMESPACE_FDB_KERNEL,
	      ...
	      u32 *default_tbl_id)
{
	...
	u32 attr_sz = ns_to_attr_sz(MLX5_FLOW_NAMESPACE_FDB_KERNEL);
	...
	*default_tbl_id = default_tbl->id;
	...
	post_attr = mlx5_alloc_flow_attr(MLX5_FLOW_NAMESPACE_FDB_KERNEL);
	if (!post_attr) {
		err = -ENOMEM;
		goto err_attr;
	}
			...
			u32 tunnel_id)
{
	 ...
	 */
	pre_attr = mlx5_alloc_flow_attr(MLX5_FLOW_NAMESPACE_FDB_KERNEL);
	if (!pre_attr) {
		err = -ENOMEM;
