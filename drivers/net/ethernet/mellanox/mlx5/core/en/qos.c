			     ...
			     struct mlx5e_qos_node *node)
{
	...
	err = mlx5e_open_txqsq(c, priv->tisn[c->lag_port][0], txq_ix, params,
			       &param_sq, sq, NULL, 0, node->hw_id, node->qid);
	if (err)
		goto err_close_cq;
		       ...
		       struct netlink_ext_ack *extack)
{
	...
	/* mlx5e_htb_root_del can't fail - alloc in advance. */
	priv->htb.final_selq = kvzalloc(sizeof(*priv->htb.final_selq), GFP_KERNEL);
	if (!priv->htb.final_selq)
		return -ENOMEM;
	...
	if (opened) {
		...
		if (!selq) {
			err = -ENOMEM;
			goto err_free_final_selq;
		}
		mlx5e_build_selq(selq, &priv->channels.params, true);
...
err_free_selq:
	kvfree(selq);
err_free_final_selq:
	kvfree(priv->htb.final_selq);
	priv->htb.final_selq = NULL;
	return err;
}

int mlx5e_htb_root_del(struct mlx5e_priv *priv)
{
	struct mlx5e_select_queue_params *selq = priv->htb.final_selq;
	struct mlx5e_qos_node *root;
	int err;
	...
	if (!WARN_ON(!selq)) {
		...
		mlx5e_build_selq(selq, &priv->channels.params, false);
		mlx5e_replace_selq(priv, selq);
	}
	priv->htb.final_selq = NULL;
