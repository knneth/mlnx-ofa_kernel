...
#include <net/arp.h>
#if defined(HAVE_FIB_INFO_NH) && defined(HAVE_FIB_NH_DEV)
				     ...
				     int out_index)
{
	...
	err = mlx5e_set_fwd_to_int_port_actions(priv, attr, route_dev,
			       ...
			       int out_index)
{
	...
	err = mlx5e_tc_tun_query_route_vport(out_dev, route_dev, &vport_num);
	if (err) {
	...
	act_id = &esw_attr->dests[out_index].src_port_rewrite_act_id;
				  ...
				  int out_index)
{
	...
	data = mlx5_eswitch_get_vport_metadata_for_set(attr->in_mdev->priv.eswitch,
						       vport_num);
	err = mlx5e_tc_match_to_reg_change(esw->dev, mod_hdr_acts,
					   VPORT_TO_REG, act_id, data);
	if (err)
		goto out;
			      ...
			      struct mlx5_flow_spec *spec)
{
	...
	attr->ip_version = mlx5e_tc_get_ip_version(spec, true);
	...
	if (attr->ip_version == 4) {
		...
		attr->dst_ip.v4 = *(__be32 *)daddr;
		attr->src_ip.v4 = *(__be32 *)saddr;
		if (!attr->dst_ip.v4 || !attr->src_ip.v4)
			return;
	}
		...
		memcpy(&attr->dst_ip.v6, daddr, ipv6_size);
		memcpy(&attr->src_ip.v6, saddr, ipv6_size);
		if (!memcmp(&attr->dst_ip.v6, &zerov6, sizeof(zerov6)) ||
		    !memcmp(&attr->src_ip.v6, &zerov6, sizeof(zerov6)))
			return;
	}
#endif
...
void mlx5e_tc_update_neigh_used_value(struct mlx5e_neigh_hash_entry *nhe)
{
	...
	if (neigh_used) {
		 ...
		 * dst ip pair
		 */
		n = neigh_lookup(tbl, &m_neigh->dst_ip, READ_ONCE(nhe->dev));
		if (!n)
			return;
...
static unsigned int mlx5e_route_tbl_get_last_update(struct mlx5e_priv *priv)
{
	...
	spin_lock_bh(&esw->offloads.route_lock);
	ret = esw->offloads.route_tbl_last_update;
	spin_unlock_bh(&esw->offloads.route_lock);
	return ret;
}
...
static inline int cmp_route_info(struct mlx5e_route_key *a,
...
static inline u32 hash_route_info(struct mlx5e_route_key *key)
{
	if (key->ip_version == 4)
...
static bool mlx5e_route_take(struct mlx5e_route_entry *r)
{
	return refcount_inc_not_zero(&r->refcnt);
}
		       ...
		       unsigned long *route_tbl_change_time)
{
	...
	spin_lock_bh(&esw->offloads.route_lock);
	*route_tbl_change_time = esw->offloads.route_tbl_last_update;
...
#if defined(HAVE_FIB_INFO_NH) && defined(HAVE_FIB_NH_DEV)
static struct mlx5e_route_entry *
mlx5e_route_lookup_for_update(struct mlx5e_priv *priv, struct mlx5e_route_key *key)
{
	struct mlx5_eswitch *esw = priv->mdev->priv.eswitch;
	...
	spin_lock_bh(&esw->offloads.route_lock);
	esw->offloads.route_tbl_last_update = jiffies;
	r = mlx5e_route_get(priv, key, hash_key);
	spin_unlock_bh(&esw->offloads.route_lock);
			     ...
			     struct mlx5e_tc_flow *flow)
{
	...
	err = mlx5e_tc_tun_route_lookup(priv, esw_attr, &parse_attr->spec);
	if (err || !esw_attr->decap_vport)
		goto out;

	key.ip_version = esw_attr->ip_version;
	if (key.ip_version == 4)
		key.endpoint_ip.v4 = esw_attr->dst_ip.v4;
	else
		key.endpoint_ip.v6 = esw_attr->dst_ip.v6;
				  ...
				  struct list_head *encap_flows)
{
	...
	if (err)
		mlx5_core_warn(priv->mdev, "Failed to update encap header err=%d", err);
	e->flags &= ~MLX5_ENCAP_ENTRY_NO_ROUTE;
				  ...
				  struct list_head *decap_flows)
{
	...
	list_for_each_entry(flow, decap_flows, tmp_list) {
		...
		parse_attr = attr->parse_attr;
		spec = &parse_attr->spec;
		err = mlx5e_tc_tun_route_lookup(priv, esw_attr, spec);
		if (err) {
...
#if defined(HAVE_FIB_NOTIFIER_INFO_HAS_FAMILY)
static struct mlx5e_tc_fib_event_data *
			 ...
			 struct fib_notifier_info *info)
{
	 ...
	 * context.
	 */
	r = mlx5e_route_lookup_for_update(priv, &key);
	if (!r)
		goto out;
	fib_work->r = r;
	dev_hold(ul_dev);
...
int mlx5e_tc_fib_event(struct notifier_block *nb, unsigned long event, void *ptr)
{
	...
	uplink_priv = container_of(nb, struct mlx5_rep_uplink_priv, fib_nb);
	...
	case FIB_EVENT_ENTRY_DEL:
		...
		if (info->family == AF_INET)
			fib_work = mlx5e_init_fib_work_ipv4(priv, ul_dev, event, info);
		else if (info->family == AF_INET6)
			fib_work = mlx5e_init_fib_work_ipv6(priv, ul_dev, event, info);
		else
#endif
			return NOTIFY_DONE;
