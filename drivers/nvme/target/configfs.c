...
static ssize_t nvmet_ns_offload_cmd_tmo_us_show(struct config_item *item,
						char *page)
{
	return sprintf(page, "%d\n", to_nvmet_ns(item)->offload_cmd_tmo_us);
}

static ssize_t nvmet_ns_offload_cmd_tmo_us_store(struct config_item *item,
						 const char *page, size_t count)
{
	struct nvmet_ns *ns = to_nvmet_ns(item);
	...
	ret = kstrtou32(page, 0, &offload_cmd_tmo_us);
	if (ret)
		return ret;

	mutex_lock(&ns->subsys->lock);
	if (ns->enabled) {
		pr_err("disable ns before setting offload_cmd_tmo_us value.\n");
		mutex_unlock(&ns->subsys->lock);
		return -EINVAL;
	}

	ns->offload_cmd_tmo_us = offload_cmd_tmo_us;
	mutex_unlock(&ns->subsys->lock);
	return count;
}

CONFIGFS_ATTR(nvmet_ns_, offload_cmd_tmo_us);
...
static struct configfs_attribute *nvmet_ns_attrs[] = {
	...
	&nvmet_ns_attr_offload_cmd_tmo_us,
