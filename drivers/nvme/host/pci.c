...
static void nvme_peer_init_resource(struct nvme_queue *nvmeq,
	...
	void (* stop_master_peer)(void *priv), void *dd_data)
{
	...
	if (mask & NVME_PEER_SQ_SZ)
		nvmeq->resource.nvme_sq_size = SQ_SIZE(nvmeq);

	if (mask & NVME_PEER_CQ_SZ)
		nvmeq->resource.nvme_cq_size = CQ_SIZE(nvmeq);

	if (mask & NVME_PEER_MEM_LOG_PG_SZ)
		nvmeq->resource.memory_log_page_size = __ffs(NVME_CTRL_PAGE_SIZE >> 12); /* memory_log_page_size is in 4K granularity */
			     ...
			     bool destroy_queues)
{
	...
	if (destroy_queues) {
		...
		if (ret)
			return ret;
		ret = adapter_delete_cq(nvmeq->dev, nvmeq->qid);
		if (ret)
			return ret;
	}
	...
	for (i = 0; i <= max; i++) {
		...
		if (nvmeq->p2p) {
			...
			if (!nvmeq->resource.in_use) {
				...
				nvme_peer_init_resource(nvmeq, mask,
							stop_master_peer,
							dd_data);
...
static int nvme_setup_irqs(struct nvme_dev *dev, unsigned int nr_io_queues)
{
	...
	if (!(dev->ctrl.quirks & NVME_QUIRK_SINGLE_VECTOR))
		irq_queues += (nr_io_queues - poll_queues - dev->num_p2p_queues);
...
static bool nvme_pci_update_nr_queues(struct nvme_dev *dev)
{
	...
	blk_mq_update_nr_hw_queues(&dev->tagset, dev->tagset.nr_hw_queues);
		...
		const struct pci_device_id *id)
{
	...
	dev->queues = kcalloc_node(dev->nr_allocated_queues + num_p2p_queues,
...
static int nvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (dev->online_queues > 1) {
		dev->ctrl.num_p2p_queues = dev->num_p2p_queues;
		nvme_alloc_io_tag_set(&dev->ctrl, &dev->tagset, &nvme_mq_ops,
				nvme_pci_nr_maps(dev), sizeof(struct nvme_iod));
		nvme_dbbuf_set(dev);
	}
