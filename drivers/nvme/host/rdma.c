		...
		const char *op)
{
	...
	if (nvme_rdma_wr_error(ctrl, wc, op) &&
		...
		req->status = cpu_to_le16((NVME_SC_ABORT_REQ |
					NVME_SC_DNR) << 1);
		nvme_try_complete_req(blk_mq_rq_from_pdu(req), req->status,
				      req->result);
	}
}
...
#ifdef CONFIG_NVFS
	if (nvme_rdma_nvfs_unmap_data(queue, rq))
		return;
#endif
...
static void nvme_rdma_sig_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	if (unlikely(wc->status != IB_WC_SUCCESS))
		nvme_rdma_req_error(req, wc, "SIG");
}
		...
		struct request *rq, struct nvme_command *c)
{
	...
	if (ret)
        ...
        ret = nvme_rdma_nvfs_map_data(queue, rq, c, &is_nvfs_io);
        if (is_nvfs_io)
                return ret;
        }
#endif
