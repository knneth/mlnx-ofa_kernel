...
static struct ib_mad_send_buf *cm_alloc_msg_rep(struct cm_id_private *cm_id_priv, bool rep)
{
	struct ib_mad_agent *mad_agent;
	...
	mad_agent = rep ? cm_id_priv->av.port->rep_agent : cm_id_priv->av.port->mad_agent;
	if (!mad_agent) {
		m = ERR_PTR(-EINVAL);
		goto out;
	}
...
{
	return cm_alloc_msg_rep(cm_id_priv, false);
}
...
cm_alloc_priv_msg_rep(struct cm_id_private *cm_id_priv, enum ib_cm_state state, bool rep)
{
	struct ib_mad_send_buf *msg;

	lockdep_assert_held(&cm_id_priv->lock);

	msg = cm_alloc_msg_rep(cm_id_priv, rep);
	if (IS_ERR(msg))
		return msg;
