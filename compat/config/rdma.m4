...
[
	...
	*)
	...
	AC_MSG_CHECKING([if xfrm_state_offload exists])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/xfrm.h>
	],[
		struct xfrm_state_offload x;
		...
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_XFRM_STATE_OFFLOAD, 1,
			  [xfrm_state_offload exists])
	],[
		AC_MSG_RESULT(no)
	])
	...
	AC_MSG_CHECKING([if FLOW_ACTION_PPPOE_PUSH exists])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/flow_offload.h>
	],[
		enum flow_action_id action = FLOW_ACTION_PPPOE_PUSH;
		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_FLOW_ACTION_PPPOE_PUSH, 1,
			  [FLOW_ACTION_PPPOE_PUSH exists])
	],[
		AC_MSG_RESULT(no)
	])

	AC_MSG_CHECKING([if FLOW_ACTION_GATE exists])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/flow_offload.h>
	],[
		enum flow_action_id action = FLOW_ACTION_GATE;
		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_FLOW_ACTION_GATE, 1,
			  [FLOW_ACTION_GATE exists])
	],[
		AC_MSG_RESULT(no)
	])
	...
	AC_MSG_CHECKING([if FLOW_ACTION_CT_METADATA exists])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/flow_offload.h>
	],[
		enum flow_action_id action = FLOW_ACTION_CT_METADATA;
		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_FLOW_ACTION_CT_METADATA, 1,
			  [FLOW_ACTION_CT_METADATA exists])
	],[
		AC_MSG_RESULT(no)
	])
	...
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_VFS_GETATTR_HAS_4_PARAMS, 1,
	...
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_FILTER_H_HAVE_XDP_BUFF, 1,
	...
	AC_MSG_CHECKING([if has sturct vfio_log_ops])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		...
		static const struct vfio_log_ops mlx5vf_pci_log_ops = {
			.log_start = NULL,
			.log_stop = NULL,
			.log_read_and_clear = NULL,
		};
	
		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_VFIO_LOG_OPS, 1,
			  [sturct vfio_log_ops exists])
	],[
		AC_MSG_RESULT(no)
	])
	...
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		...
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_PAGE_POOL_GET_DMA_ADDR, 1,
	...
	AC_MSG_CHECKING([if uapi/linux/xfrm.h has XFRM_OFFLOAD_FULL])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <uapi/linux/xfrm.h>
	],[
		int a = XFRM_OFFLOAD_FULL;
		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_XFRM_OFFLOAD_FULL, 1,
			  [XFRM_OFFLOAD_FULL is defined])
	],[
		AC_MSG_RESULT(no)
	])
	...
	AC_MSG_CHECKING([if blkdev.h has blk_queue_update_readahead])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <linux/blkdev.h>
	],[
		blk_queue_update_readahead(NULL);

		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_BLK_QUEUE_UPDATE_READAHEAD, 1,
				[blk_queue_update_readahead is defined in blkdev.h])
	],[
		AC_MSG_RESULT(no)
	])
	...
	AC_MSG_CHECKING([if struct macsec_context has prepare])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/macsec.h>
	],[
		struct macsec_context mctx;
		...
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_MACSEC_CTX_PREPARE, 1,
			[struct macsec_context has prepare])
	],[
		AC_MSG_RESULT(no)
	])
